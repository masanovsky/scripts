    ...
    function(player)
        ...
        if imgui.Begin(u8'Сейф', renderWindow, imgui.WindowFlags.NoResize + imgui.WindowFlags.NoCollapse) then
            ...
            elseif tab == 2 then
                ...
                imgui.PopItemWidth()
                if imgui.Checkbox(u8'Бронижелет', m_arm) then
            ...
            elseif tab == 5 then
                if imgui.Checkbox(u8'Скрыть уведомления о взятии оружия', fctext) then
                    cfg.SETTINGS.fctext = fctext[0]
                    inicfg.save(cfg, 'safe.ini')
                end
                if imgui.Checkbox(u8'Скрыть уведомления о спавне транспорта', vehtext) then
                    cfg.SETTINGS.vehtext = vehtext[0]
                    inicfg.save(cfg, 'safe.ini')
                end
                if imgui.Checkbox(u8'Коллизия на игроков в интерьере', pintcol) then
                    cfg.SETTINGS.pintcol = pintcol[0]
                    inicfg.save(cfg, 'safe.ini')
                end
                if imgui.Checkbox(u8' Коллизия на игроков в обычном мире', pcol) then
            ...
            if drugtimer <= 0 then
                ...
                if excludedParam then
                    ...
                    if value and value > 150 then
                        sampAddChatMessage('Взятие ограничено 150 единицами наркотиков', 0xFF0000)
                        return false
                    end
                    return true
                end
...
function sampev.onServerMessage(color, text)
    ...
    if text:find('.+ {FFFFFF}.+ (.+) припарковал.+т/c Sultan из слота 1') then
        nick1 = text:match('masanovskiy.+ {FFFFFF}.+ (.+) припарковал.+т/c Sultan из слота 1')
        time1 = os_time()
    end
    if text:find('.+ {FFFFFF}.+ (.+) припарковал.+т/c Sultan из слота 2') then
        nick2 = text:match('masanovskiy.+ {FFFFFF}.+ (.+) припарковал.+т/c Sultan из слота 2')
        time2 = os_time()
    end
    if text:find('.+ {FFFFFF}.+ (.+) припарковал.+т/c Sultan из слота 3') then
        nick3 = text:match('masanovskiy.+ {FFFFFF}.+ (.+) припарковал.+т/c Sultan из слота 3')
        time3 = os_time()
    end
    if text:find('.+ {FFFFFF}.+ (.+) припарковал.+т/c Sultan из слота 4') then
        nick4 = text:match('masanovskiy.+ {FFFFFF}.+ (.+) припарковал.+т/c Sultan из слота 4')
        time4 = os_time()
    end
    if text:find('.+ {FFFFFF}.+ (.+) припарковал.+т/c Sultan из слота 5') then
        nick5 = text:match('masanovskiy.+ {FFFFFF}.+ (.+) припарковал.+т/c Sultan из слота 5')
        time5 = os_time()
    end
...
if dialogTitle:find('{FFFFFF}Перечень | {ae433d}Автомобили') then
    ...
    lua_thread.create(function()
        ...
        for line in string.gmatch(dialogText, '[^\n]+') do
            ...
            if lineIndex == 0 and #columns >= 4 then
                columns[4] = columns[4]:gsub("Доступ", "Заспавнил")
            elseif #columns >= 4 then
                ...
                if timeValue and timeValue > 0 then
                    formattedTime = string.format("%s [%d сек]", nickname, os_time() - timeValue)
                else
    ...
    b3 = dialogTitle:gsub('{FFFFFF}Перечень | {ae433d}Автомобили', '{ae433d}Используйте клавиши {FFFFFF}" 1-6 " {ae433d}для быстрой парковки')
