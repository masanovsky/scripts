...
function main()
    ...
    if not pcall(check_update) then print('Íå óäàëîñü ïðîâåðèòü îáíîâëåíèå') end
    while not sampIsLocalPlayerSpawned() do wait(0) end
    print('v0.2')
    sampAddChatMessage(' [SAFE] {ffffff}Ñêðèïò óñïåøíî çàãðóæåí! Êëàâèøà àêòèâàöèè: F12', 0x177517)
    while true do
        wait(0)
...
function sampev.onServerMessage(color, text)
    if (color == -1347440726) then
        if text:find("Ñêëàä âàøåé ñåìüè çàêðûò") or text:find('Âû äîëæíû íàõîäèòüñÿ â ïðèâÿçàííîì ê ñåìüå äîìå') then
            fsafe = false
        end
    elseif color == -858993409 then
        if text:find("Ïèí-êîä íå ñîâïàë") then
            fsafe = false
        elseif text:find('Äîñòóïíûå ïàðàìåòðû: ') then
            sampAddChatMessage(' Äîñòóïíûå ïàðàìåòðû: drug', 0xcccccc)
            return false
        end
    elseif color == -3407617 and text:find("Ñåéô îòêðûâàåòñÿ") and fama then
        open = true
        fama = false
    end
end
