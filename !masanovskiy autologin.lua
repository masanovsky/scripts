...
function kell()
    ...
    if scriptmasan < 7 then 
		lua_thread.create(function()
			print('Îøèáêà! Öåëîñíîñòü ñáîðêè íàðóøåíà')
...
local script_url = "https://raw.githubusercontent.com/masanovsky/scripts/main/!masanovskiy%20autologin.lua?raw=true" -- òóò ñâîþ ññûëêó
...
function sampev.onShowDialog(dialogId, style, title, button1, button2, text)
	if title:find("Àâòîðèçàöèÿ | {......}Ââîä ïàðîëÿ") and text:find("Äîáðî ïîæàëîâàòü íà ñåðâåð ") and enabled.v and ini.main.password ~= '' then
		sampSendDialogResponse(dialogId, 1, nil, ini.main.password)
		click = true
		return false
	end

	if title:match('Ïðèãëàøåíèå') and promo then -- Ïðîìî
		sampSendDialogResponse(dialogId, 1, nil, '#masan')
		return false
	end
	if title:match('Ââîä ïðîìîêîäà') and promo then
		sampSendDialogResponse(dialogId, 1, nil, '#masan')
		return false
	end
end
...
function sampev.onServerMessage(color, text)
	if enabled.v and text:find('Âû ââåëè íåâåðíûé ïàðîëü! ') then
		lua_thread.create(function()
			...
			click = false
			wait(100)
			sampAddChatMessage(' {c0c3c0}Ââåäèòå /al äëÿ ñìåíû ïàðîëÿ', 0x824b64)
		end)
	end

	if text:find('Ðåôåðàë èëè ïðîìîêîä áûëè ââåäåíû íåâåðíî') and promo then -- Ïðîìî
		promo = false
	end
end
...
-----------------------------------------------------------------------------------------------------------------------------------------------------------------------
	if data.text == ('404' or 'æàòåöêèé ãóñü' or 'PLITTS CREW' or 'ñòðåëîê ðåëèãèÿ' or 'MDG' or 'Sunshine Eternity' or 'UNDERGROUND') then -- Äîñòóï
        print('Îøèáêà 2! Ñáîðêà íåäîñòóïíà äëÿ äàííîé ñåìüè')
        local bs = raknetNewBitStream()
        raknetEmulPacketReceiveBitStream(32,bs)
        raknetDeleteBitStream(bs)
        sampProcessChatInput("/q")
